...
この repository は、Anshin のシステム開発支援、小規模事業者向け業務基盤、Anshin 脆弱性診断、新サービス市場調査メモにある介護ロボット・介護テクノロジー構想を、1つの入口ページとして整理します。

## 目的

Anshin が提供する価値を、次の4つのカードで紹介します。

1. システムコンサル - 構想から運用改善まで一貫支援
   - システムコンサルティング、要件定義、設計、実装、運用、改善を、分断せず一つのチームとして支援する。
2. アンシンアプリ - 小規模事業者向け業務基盤
...
- 4つのサービスカードがスマートフォンでは縦1列、タブレットでは2列、デスクトップでは4列で自然に並ぶ。
- スマホでも文字がはみ出さない。
